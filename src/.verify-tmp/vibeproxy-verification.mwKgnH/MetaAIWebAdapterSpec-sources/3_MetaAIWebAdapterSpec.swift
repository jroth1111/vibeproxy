...
struct MetaAIWebAdapterSpec {
    static func main() {
        ...
        run("meta web adapter preserves reasoning text and metadata markers alongside visible text summary content", recorder: recorder) {
            let request = #"""
            {
              "model": "muse-spark",
              "messages": [
            ...
            do {
                ...
                expectEqual(
                    parsed.prompt,
                    """
                    Assistant: Checked repohidden chain of thought{"step":1}

                    User: Answer briefly
                    """,
                    "reasoning text and metadata markers should be preserved alongside visible summary text",
                    recorder: recorder
                )
                ...
                recorder.recordFailure("meta web adapter should preserve reasoning text and metadata markers alongside visible text: \(error)")
            }
        }

        run("meta web adapter preserves metadata-only typed transcript content instead of rejecting the request", recorder: recorder) {
            let request = #"""
            {
              "model": "muse-spark",
              "messages": [
            ...
            do {
                ...
                expectEqual(
                    parsed.prompt,
                    """
                    Assistant: hidden chain of thought{"step":1}

                    User: Answer briefly
                    """,
                    "metadata-only typed transcript content should be preserved instead of excluding the Meta bridge",
                    recorder: recorder
                )
                expectEqual(parsed.isNewThread, false, "metadata-only assistant context should still force follow-up mode when preserved", recorder: recorder)
            } catch {
                recorder.recordFailure("meta web adapter should preserve metadata-only typed transcript content instead of rejecting the request: \(error)")
            }
        }
